      ...
      <section className="classroom-stage rich-classroom" aria-live="polite">
        ...
        {stage === 5 && (
          <div className="stage-content open-stage">
            ...
            <textarea maxLength={1200} value={openResponse} onChange={(event) => { setOpenSubmitted(false); setOpenResponse(event.target.value); }} placeholder="在这里写下你的发现和依据……" aria-label="开放表达答案" />
        ...
        {stage === 7 && (
          <div className="stage-content extension-stage">
            ...
            <section className={`question-maker ${questionMakerReady ? "ready" : ""}`}><span className="eyebrow">我来当出题人 · {course.lesson.kindLabel}工坊</span><h2>{course.lesson.questionStudio.mission}</h2><p>好题不能只靠猜：题目要清楚，答案要能核对，还要解释为什么。</p><div className="question-stems"><strong>选择一个题干灵感</strong>{course.lesson.questionStudio.stems.map((stem) => <button key={stem} onClick={() => setCustomQuestion(stem)}>{stem}</button>)}</div><label><strong>1 · 我的题目</strong><textarea maxLength={240} value={customQuestion} onChange={(event) => setCustomQuestion(event.target.value)} placeholder={course.lesson.gradeBand === "upper" ? "试试加入条件、比较或反例……" : "围绕本课知识，问一个需要观察或思考的问题……"} /></label><label><strong>2 · 参考答案</strong><textarea maxLength={320} value={customAnswer} onChange={(event) => setCustomAnswer(event.target.value)} placeholder="写出可以核对的参考答案……" /></label><label><strong>3 · 判断依据</strong><textarea maxLength={320} value={customReason} onChange={(event) => setCustomReason(event.target.value)} placeholder="说明答案为什么成立，要回到哪条知识或证据……" /></label><aside><strong>本课型好题标准：</strong>{course.lesson.questionStudio.qualityCheck}</aside><p className="maker-status">{questionMakerReady ? "✓ 出题卡完成：请把题目给家人做，再根据他的回答修改题目。" : "完成题目、答案和依据后，你就从答题者升级为出题人。"}</p></section>
