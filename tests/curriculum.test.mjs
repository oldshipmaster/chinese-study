...
test("every grade-one and grade-two course has unique curated lesson content", async () => {
  ...
  const curatedGrades = curriculum.slice(
    ...
    curriculum.indexOf('bookId: "g3-upper"'),
  );
  ...
  assert.ok(courseIds.length >= 170, `expected at least 170 curated courses, got ${courseIds.length}`);
